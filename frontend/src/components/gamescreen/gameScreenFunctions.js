...
export const gameScreenFunctions = () => {
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('victoriesPlayerOne', gameScreenState.victoriesPlayerOne);
      sessionStorage.setItem('victoriesPlayerTwo', gameScreenState.victoriesPlayerTwo);
      sessionStorage.setItem('currentPlayer', gameScreenState.currentPlayer);
    } catch (error) {
      console.error(error);
    }
  }, [gameScreenState.victoriesPlayerOne, gameScreenState.victoriesPlayerTwo, gameScreenState.currentPlayer]);
  ...
  useEffect(() => {
    if (gameScreenState.winner) {
      const colorClase = gameScreenState.winner === homeState.namePlayerOne ? homeState.selectedColorOne : homeState.selectedColorTwo;
      handleEffectClick()
      Swal.fire({
      ...
      setTimeout(() => {    
        ...
        sessionStorage.setItem('winningBoard' ,JSON.stringify(gameScreenState.board));
        resetBoard();
      }, 1500);
